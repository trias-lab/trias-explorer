...
def save2db():
    try:
        ...
        if not db_last_block_hash:
            block_hash = mongo_util.get_first_block_hash()
        else:
            block_hash = mongo_util.get_next_block_hash(db_last_block_hash)

        if not block_hash:
            logger.info('no new block in chain, wait new block')
            return False
        ...
        block_info = mongo_util.get_block_with_hash(block_hash)
        if not block_info:
            logger.error('get block info error, hash %s' % block_hash)
            return False
        ...
        for tx in block_info['block']['transactions']:
            ...
            sql =  "INSERT INTO transaction (blockHash, timestamp, blockNumber, hash, source, `to`, value) "\
                        "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')" %\
                        (block_hash, block_timestamp, block_number, tx['id'], source, to, asset_str)
