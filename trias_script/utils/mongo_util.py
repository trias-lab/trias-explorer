...
def get_first_block_hash():
    try:
        conn = MongoClient(CONF.bdb_mongo_ip, CONF.bdb_mongo_port)
        db = conn.get_database('octachain')
        doc = db.get_collection('bigchain').find_one({"block.transactions.operation":'GENESIS'}, sort=[('_id', 1)])
        if doc:
            hash = doc['id']
            return hash
        else:
...
def get_block_by_height(height):
    try:
        conn = MongoClient(CONF.bdb_mongo_ip, CONF.bdb_mongo_port)
        db = conn.get_database('octachain')
        docs = list(db.get_collection('bigchain').find({}, sort=[('_id', 1)], limit=1, skip=height-1))
        if len(docs) > 0:
            return docs[0]
        else:
            return None
